...
> 
> 하지만 `Association 'Notification.sender' targets the type 'Member' which does not belong to the same persistence unit` 메시지와 함께 애플리케이션이 실행되지 않는 문제가 발생했습니다.
...
**✅ 예외 발생**
```
Caused by: org.hibernate.AnnotationException: Association 'com.xowns.celfeed.domain.notification.Notification.sender' targets the type 'com.xowns.celfeed.domain.basic.Member' which does not belong to the same persistence unit
```
...
> 
> (참고로 클라이언트에 제공하는 알림 메시지에는 sender의 닉네임이 필요합니다.)
...
- 단점
    - 기존에는 페치 조인을 통해 알림을 조회함과 동시에 sender의 닉네임도 함께 조회했습니다.
...
- Notification 테이블에 닉네임 컬럼을 추가해서 알림 저장 시점에 sender의 닉네임도 함께 저장합니다.
- 단점
